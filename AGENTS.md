...
7. **Only auto-reconnect on `ESP_A2D_DISC_RSN_ABNORMAL`** (signal loss). `ESP_A2D_CONNECTION_STATE_EVT`'s `conn_stat.disc_rsn` distinguishes a graceful AVDTP/ACL teardown (`NORMAL` — includes the phone explicitly disconnecting, or our own `disconnect()`/`disable()`) from an unexpected link drop (`ABNORMAL`). Scheduling `reconnect_at_` on `NORMAL` fights the phone's explicit request instead of respecting it — the phone will typically accept the reconnect, making it look like the device "won't let go". `start_discovery_()` still runs either way so the device stays passively connectable.
